...
def training_generator(temp):
    ...
    patch_num=20
    patch_num1=16
    x_data=np.zeros([45000,64,64,16,3])
    y_data=np.zeros([45000,64,64,16])
    time=-1
    tr_X1='/HGG/'
    ...
    for sub in range (0,228):
        ...
        if sub<168:
            ...
            P1=nib.load('/step1/out/HGG/'+n+'.nii.gz').get_data() #step 1 result
        else:
            n=lst1[int(temp[sub])-1]
            T1c=nib.load('/LGG/'+n+'/T1c.nii').get_data()
            Mask=nib.load('/LGG/'+n+'/seg.nii').get_data()
            
            P1=nib.load('/step1/out/LGG/'+n+'.nii.gz').get_data() #step 1 result
        Mask[Mask==4]=3
        P1[Mask>0]=0
        for flip_num in range (6):  
            T1c_temp=np.copy(T1c)
            label_vols=np.copy(Mask)
            label=np.copy(P1)
            
            if flip_num-2>0:
                    T1c_temp=np.flip(T1c_temp,axis=flip_num-3)
                    label_vols=np.flip(label_vols,axis=flip_num-3)
                    label=np.flip(label,axis=flip_num-3)
                    
            if flip_num==2:
                    noise = np.random.normal(0,1,T1c_temp.shape)
                  
                    T1c_temp = np.add(T1c_temp, noise)
            
            min_D_s, max_D_e, min_H_s, max_H_e, min_W_s, max_W_e = cut_edge(label,margin)
            inputs_T1c = T1c_temp[min_D_s:max_D_e + 1, min_H_s: max_H_e + 1, min_W_s:max_W_e + 1, None]
            labels = label_vols[min_D_s:max_D_e + 1, min_H_s: max_H_e + 1, min_W_s:max_W_e + 1]
            labelp = label[min_D_s:max_D_e + 1, min_H_s: max_H_e + 1, min_W_s:max_W_e + 1]
             
            inputs = np.concatenate((inputs_T1c,inputs_T1c, inputs_T1c), axis=3)
            if flip_num==1:
                inputs=np.transpose(inputs,(1,0,2,3))
                labels=np.transpose(labels,(1,0,2))
                labelp=np.transpose(labelp,(1,0,2))
            ...
            for ind in range(patch_num):
                if labelp[rnd0[ind], rnd1[ind], rnd2[ind]]>0:
                    time+=1
...
##                
##                P1=nib.load('/step1/out/HGG/'+n+'.nii.gz')
##                P1 = resample_to_img(scale_image(P1,0.75),
##                                     P1, interpolation="nearest").get_data()
##            else:
##                n=lst1[int(temp[sub])-1]
##            
...
##                                     Mask, interpolation="nearest").get_data()
##                
##                P1=nib.load('/step1/out/LGG/'+n+'.nii.gz')
##                P1 = resample_to_img(scale_image(P1,0.75),
##                                     P1, interpolation="nearest").get_data()
##            Mask[Mask==4]=3
##            P1[Mask>0]=0
...
##                label=np.copy(P1)
##                if flip_num>0:
##                        T1c_temp=np.flip(T1c_temp,axis=flip_num-1)
##                        label_vols=np.flip(label_vols,axis=flip_num-1)
##                        label=np.flip(label,axis=flip_num-1)
##                    
##                   
##                min_D_s, max_D_e, min_H_s, max_H_e, min_W_s, max_W_e = cut_edge(label,margin)
##                inputs_T1c = T1c_temp[min_D_s:max_D_e + 1, min_H_s: max_H_e + 1, min_W_s:max_W_e + 1, None]
...
##                for ind in range(patch_num):
##                    #if labelp[rnd0[ind], rnd1[ind], rnd2[ind]]>0:
##                    if len(np.unique(labelp[rnd0[ind]:(rnd0[ind]+patch_size), rnd1[ind]:(rnd1[ind]+patch_size), rnd2[ind]:(rnd2[ind]+patch)]))>1:
##                        time+=1
##                        x_data[time]=inputs[rnd0[ind]:(rnd0[ind]+patch_size), rnd1[ind]:(rnd1[ind]+patch_size), rnd2[ind]:(rnd2[ind]+patch),:3]
##                        y_data[time]=labels[rnd0[ind]:(rnd0[ind]+patch_size), rnd1[ind]:(rnd1[ind]+patch_size), rnd2[ind]:(rnd2[ind]+patch)]
##
...
def validate_generator(temp1):
    ...
    lst1 = os.listdir(tr_X2)
    lst1.sort()
    for sub in range (0,57):
        if sub<42:
            n=lst[int(temp1[sub])-1]
          
            T1c=nib.load('/HGG/'+n+'/T1c.nii').get_data()
            Mask=nib.load('/HGG/'+n+'/seg.nii').get_data()
            P1=nib.load('/HGG/'+n+'.nii.gz').get_data()
        else:
            n=lst1[int(temp1[sub])-1]
            T1c=nib.load('/LGG/'+n+'/T1c.nii').get_data()
            Mask=nib.load('/LGG/'+n+'/seg.nii').get_data()
            P1=nib.load('/step1/out/LGG/'+n+'.nii.gz').get_data()
        Mask[Mask==4]=3
